...
<body>
	<?php
	 ...
	 $rows=$mysqli->query("SELECT date_sdachi, id_student, id_subject, ocenka FROM vedomost WHERE
	id_vedomost=".$_GET['id_vedomost']);
	 ...
	 $id=$_GET['id_vedomost'];
	 $data_sdachi = date('d.m.Y',strtotime($st['date_sdachi']));
	 $id_student = $st['id_student'];
	 $id_subject = $st['id_subject'];
	 $ocenka = $st['ocenka'];
	 }
	print "<form action='save_editVedomost.php' metod='get'>";
	print "Дата сдачи: <input name='date_sdachi' size='50' type='text'
	value='".$data_sdachi."'>";
